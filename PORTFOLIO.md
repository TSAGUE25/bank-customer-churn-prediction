...
```markdown
# Bank Customer Churn Prediction
## XGBoost + SHAP pour prédire et expliquer le churn bancaire

> **Auteur :** Emmanuel TSAGUE | **Données :** simulées

## Objectif
Identifier les clients à risque de départ avec recall maximisé,
en gérant le déséquilibre de classes et en expliquant les prédictions.

## Résultats (simulés)
AUC = 0.87 · Recall churn = 0.79 · F1 = 0.70 · Économie simulée : 485 000 €
```
...
> Prédiction du churn bancaire sur 15 000 clients simulés : gestion du déséquilibre (16 % churn) par class_weight="balanced" et scale_pos_weight XGBoost, pipeline sklearn complet, optimisation du seuil de décision par courbe Précision-Rappel (F1 maximal), interprétabilité SHAP (beeswarm + waterfall), analyse coût FP/FN — AUC simulé 0,87, Recall 0,79.

---

## 12. VERSION ENTRETIEN

"J'ai traité un problème de churn bancaire avec 16 % de churners — un déséquilibre classique. La première erreur à éviter : optimiser l'accuracy globale, qui donne 84 % même en prédisant toujours 'non-churn'. J'ai utilisé XGBoost avec scale_pos_weight pour pénaliser davantage les erreurs sur la classe minoritaire. La deuxième décision clé : le seuil de décision. Par défaut à 0,5, le recall est trop faible — on rate trop de vrais churners. J'ai optimisé le seuil par courbe Précision-Rappel pour maximiser le F1. Enfin, j'ai utilisé SHAP pour expliquer les prédictions individuelles — ça permet de justifier les actions de rétention auprès du business : 'ce client est à risque principalement parce qu'il est inactif depuis 8 mois et n'a qu'un seul produit'."

---

## 13. POST LINKEDIN

**Un modèle de churn à 84 % de précision... qui ne détecte aucun churner.**

C'est le piège classique du déséquilibre de classes.

16 % de clients churneront dans les 6 mois. Un modèle qui dit toujours "pas de churn" a 84 % d'accuracy. Mais un recall de 0 %. Inutile.

Trois solutions implémentées dans ce cas d'usage :
1. class_weight="balanced" pour pénaliser les erreurs sur la classe minoritaire
2. XGBoost scale_pos_weight pour le même effet
3. Optimisation du seuil de décision par courbe Précision-Rappel

Résultat simulé : recall 79 % vs 12 % baseline, AUC 0,87.

Et pour expliquer les prédictions au business : SHAP. "Ce client a 73 % de probabilité de churn principalement parce qu'il n'est pas membre actif et n'a qu'un seul produit."

`#Churn` `#MachineLearning` `#XGBoost` `#SHAP` `#Bancaire` `#DataScience`

---

## 14. QUESTIONS D'ENTRETIEN

**Q : Pourquoi ne pas utiliser l'accuracy pour évaluer un modèle de churn ?**
> L'accuracy est trompeuse avec des classes déséquilibrées. Si 84 % des clients ne churneront pas, prédire toujours "pas de churn" donne 84 % d'accuracy. Pourtant le modèle rate 100 % des churners. On utilise recall (ne pas rater de churners), précision (ne pas gaspiller les offres) et F1 (équilibre des deux).

**Q : Comment SMOTE crée-t-il des exemples synthétiques ?**
> SMOTE sélectionne un exemple minoritaire, choisit un de ses K voisins les plus proches, et crée un nouveau point en interpolant linéairement entre les deux. Ce n'est pas une duplication mais une interpolation — les nouveaux exemples sont réalistes mais pas identiques aux originaux.
...
*Fin du document — Emmanuel TSAGUE — CAS 11 — Churn Bancaire*
